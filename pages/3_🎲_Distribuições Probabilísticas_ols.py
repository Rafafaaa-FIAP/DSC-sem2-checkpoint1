...
#     st.title('Análise de Desmatamento com Distribuição Binomial')

#     # Opções de estados disponíveis na base de dados (exceto coluna de referência e total)
#     estados = [col for col in data.columns if col not in ['referencia', 'area_total_desmatamento']]

#     # Selecionar estado
#     estado_selecionado = st.selectbox('Selecione o estado para análise:', estados)
#     estado_data = data[estado_selecionado]

#     # Calcular a média histórica de desmatamento do estado selecionado
#     mean_deforestation_estado = estado_data.mean()

#     # Parâmetros da análise
#     n = 10000  # Número máximo de km² possíveis de serem desmatados em um ano
#     p = mean_deforestation_estado / n  # Probabilidade de sucesso por km²

#     # Input do usuário
#     area_limite = st.number_input('Insira o limite de desmatamento a ser analisado (km²):', value=1000)

#     # Calcular a probabilidade
#     probability_greater_than_limit = (1 - binom.cdf(area_limite, n, p)) * 100  # Convertido para porcentagem

#     # Mostrar resultados
#     st.write(f'Média histórica de desmatamento em {estado_selecionado}: {mean_deforestation_estado:.2f} km²')
#     st.write(f'Probabilidade de desmatar mais de {area_limite} km² em um ano: {probability_greater_than_limit:.2f}%')

#     # Gerar tabela de probabilidades
#     areas = np.arange(0, n, 500)
#     probabilities = [binom.pmf(area, n, p) for area in areas]
#     prob_table = pd.DataFrame({'Área Desmatada (km²)': areas, 'Probabilidade': probabilities})
#     st.write('### Tabela de Probabilidades')
#     st.write(prob_table)

#     # Plotar gráfico da distribuição usando Plotly
#     fig = px.bar(prob_table, x='Área Desmatada (km²)', y='Probabilidade',
#                     title=f'Distribuição Binomial do Desmatamento - {estado_selecionado}',
#                     labels={'Área Desmatada (km²)': 'Área Desmatada (km²)', 'Probabilidade': 'Probabilidade'},
